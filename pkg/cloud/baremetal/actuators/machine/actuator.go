...
// Get cloud init data for a machine
func getCloudInit(machineName string) []byte {

	home, _ := os.UserHomeDir()

	if machineName == "master" {
		cloudInit, err := ioutil.ReadFile(home + "/master-user-data.yaml")
		if err != nil {
			fmt.Printf("cloudInit.Get err   #%v ", err)
		}
		return cloudInit
	} else {
		cloudInit, err := ioutil.ReadFile(home + "/worker-user-data.yaml")
		if err != nil {
			fmt.Printf("cloudInit.Get err   #%v ", err)
		}
		return cloudInit
...
func (a *Actuator) Create(ctx context.Context, cluster *machinev1.Cluster, machine *machinev1.Machine) error {
	...
	if machine.Name == "master" {
		masterCloudInitData := getCloudInit("master")
		...
		workerCloudInitData := getCloudInit("worker")
